...
uint8_t LObject::getLengthAt(uint8_t index) {
    ...
    if (getTypeAt(index) == T_STRING || getTypeAt(index) == T_ARRAY) {
        return this->dataTable[this->getItemCount() + this->strNum(index) + INDEX_TABLE_OFFSET];
    } else {
...
uint8_t LObject::setLengthAt(uint8_t index, uint8_t length) {
    ...
    this->dataTable[this->getItemCount() + this->strNum(index) + INDEX_TABLE_OFFSET] = length;
    return length;
}
...
uint16_t LObject::setStrAt(uint8_t index, char *str, uint16_t stringLen) {
    ...
    for (uint16_t i = 0; i < stringLen && i < maxLength; i++) {
        this->dataTable[this->indexOf(index) + i] = static_cast<uint8_t>(str[i]);
    }
...
uint16_t LObject::getStrAt(uint8_t index, char *str, uint16_t stringLen, bool nullTerminate) {
    ...
    if (str == NULL) {
        return this->getLengthAt(index) + nullTerminate ? 1 : 0; // For null terminator when querying buffer size.
    }
...
uint16_t LObject::getSize() {
    return this->indexOf(this->getItemCount());
}
...
uint16_t LObject::makeImpl(GenericBuffer<uint8_t> &buffer, const char *fmt, va_list argp) {
    ...
    for(uint8_t i = 0; i < o.getItemCount(); i++) {
        switch(o.getTypeAt(i)) {
            ...
            case LObject::T_ARRAY:
                va_arg(argp, int); // Skip copying this size but move the buffer.
                break;
            case LObject::T_INT8:
